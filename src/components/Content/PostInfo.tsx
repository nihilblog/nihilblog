...
import { FaFolderOpen, FaPaintBrush, FaTag } from 'react-icons/fa';
                  ...
                  <span>카테고리</span>
                  {
                    categories.map((category) => (
                      <Link key={`category-${category}`} passHref href={`/categories/${category}`}>
                        <a className='post-info-category'><FaFolderOpen /><strong>{category}</strong></a>
                      </Link>
                    ))
                  }
                </InfoP>
                  ...
                  <span>태그</span>
                  {
                    tags.map((tag) => (
                      <Link key={`tag-${tag}`} passHref href={`/tags/${tag}`}>
                        <a className='post-info-tag'><FaTag /><strong>{tag}</strong></a>
                      </Link>
                    ))
                  }
                </InfoP>
                ...
                <span>키워드</span>
                {
                  keywords.map((keyword) => (
                    <Link key={`keyword-${keyword}`} passHref href={`/illust/keywords/${keyword}`}>
                      <a className='post-info-keyword'><FaPaintBrush /><strong>{keyword}</strong></a>
                    </Link>
                  ))
                }
              </InfoP>
